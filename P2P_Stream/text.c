...
//	}while(sConnect == -1); // 1 = connect without timeout
	...
	printf("[text] AndroidInfo_Text = %s", androidInfo_SendText);
	rval = _text_receive_parse_remote_data(agent, stream_id, 1, androidInfo_SendText);
...
static int  _text_receive_print_local_data(NiceAgent *agent, guint stream_id, guint component_id)
{
	...
	mInfo_SendText = (gchar*)malloc(181*sizeof(gchar));

	//printf("%s %s", local_ufrag, local_password);
	sprintf(mInfo_SendText, "%s %s", local_ufrag, local_password);
	for (item = cands; item; item = item->next) {
		...
		sprintf(mInfo_SendText + strlen(mInfo_SendText), " %s,%u,%s,%u,%s",
		c->foundation,
		c->priority,
		ipaddr,
		nice_address_get_port(&c->addr),
    ...
    gpointer data)
{
	...
	printf ("AndroidInfo_Text = %s\nagent = %d\nstream_id = %d\nline = %s\n", androidInfo_SendText, agent, RpiData_Text->streamID, line);
	nice_agent_send(agent, RpiData_Text->streamID, 1, strlen(line), line);
	g_free (line);
	printf("> ");
	fflush (stdout);
	} else {
...
static int _text_receive_ClientThread()
{
	...
	memcpy(temp,mInfo_SendText,sizeof(temp));
	...
	if(recv(global_socket, buffer, 181, NULL))
	{
		...
		{
			androidInfo_SendText = (gchar*)malloc(sizeof(gchar)*181);
			memcpy(androidInfo_SendText,temp1,sizeof(temp1));
			flag_text = 1;
			return 0;
		}
	}
